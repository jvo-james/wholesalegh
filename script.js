...
WGH.bindProductCards = root=>{
  (root||document).querySelectorAll('[data-product-card]').forEach(card=>{
    ...
    buttons.forEach(b=>{
      b.addEventListener('mouseenter',()=>set(b));
      b.addEventListener('click',e=>{
        e.preventDefault();e.stopPropagation();touched=true;set(b);
      });
    });

    // Desktop product-card hover changes the product colour AND its active swatch.
    // The second transparent image is disabled in CSS; JS owns the state so image/swatch never disagree.
