(async()=>{
  ...
  const colourSlug=v=>String(v||'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');
  const colourHex=c=>product.colourHexes?.[c]||WGH.colourValue(c);
  ...
  let qty=1;
  let variants=[{colour:defaultColour,size:product.sizes[1]||product.sizes[0],quantity:1}];

  document.title=`${product.name} | The Wholesale Ghana`;
  const canonical=document.querySelector('link[rel="canonical"]');
  if(canonical)canonical.href=`https://thewholesalegh.shop/product.html?id=${encodeURIComponent(product.id)}`;
  try{let rv=JSON.parse(localStorage.getItem('wgh_recently_viewed')||'[]').filter(x=>x!==product.id);rv.unshift(product.id);localStorage.setItem('wgh_recently_viewed',JSON.stringify(rv.slice(0,8)))}catch{}

  $('[data-product-name]').textContent=product.name;
  $('[data-breadcrumb-name]').textContent=product.name;
  ...
  function gallery(){
    ...
    const root=$('[data-product-gallery]');
    const special=product.id==='sculpted-high-neck-hugger-dress'&&colour==='Black'&&imgs.length>1;
    if(special){
      root.innerHTML=`<div class="special-black-gallery" data-special-gallery><figure class="single-product-image"><img src="${imgs[0]||''}" alt="${product.name} in ${colour}"></figure><figure class="single-product-image special-model-image"><img src="${imgs[1]||''}" alt="${product.name} Black styled on model"></figure></div><div class="special-gallery-pagination" aria-label="Product image position"><button type="button" class="active" aria-label="View product image"></button><button type="button" aria-label="View styled image"></button></div>`;
      requestAnimationFrame(()=>{
        const track=root.querySelector('[data-special-gallery]'),dots=[...root.querySelectorAll('.special-gallery-pagination button')];
        if(!track||!dots.length)return;
        let raf=0;
        track.addEventListener('scroll',()=>{cancelAnimationFrame(raf);raf=requestAnimationFrame(()=>{const i=Math.max(0,Math.min(dots.length-1,Math.round(track.scrollLeft/Math.max(1,track.clientWidth))));dots.forEach((d,n)=>d.classList.toggle('active',n===i))})},{passive:true});
        dots.forEach((d,i)=>d.onclick=()=>track.scrollTo({left:track.clientWidth*i,behavior:'smooth'}));
      });
    }else{
      root.innerHTML=`<figure class="single-product-image"><img src="${imgs[0]||''}" alt="${product.name} in ${colour}"></figure>`;
    }
  }
  ...
  function renderVariants(){
    const root=$('[data-variant-builder]');
    root.innerHTML=variants.map((v,i)=>`
      <article class="wholesale-mix-card" data-variant-index="${i}">
        <div class="wholesale-mix-card-head">
          <div><span>Combination ${i+1}</span><strong>${v.colour} / ${v.size}</strong></div>
          ${variants.length>1?`<button type="button" class="remove-variant" data-v-remove aria-label="Remove combination ${i+1}"><i class="fa-solid fa-xmark"></i></button>`:''}
        </div>
        <div class="wholesale-mix-fields">
          <label><span>Colour</span><select data-v-colour>${product.colours.map(c=>`<option ${c===v.colour?'selected':''}>${c}</option>`).join('')}</select></label>
          <label><span>Size</span><select data-v-size>${product.sizes.map(s=>`<option ${s===v.size?'selected':''}>${s}</option>`).join('')}</select></label>
        </div>
        <div class="wholesale-quantity-row">
          <div><span>Quantity</span><small>Pieces in this colour and size</small></div>
          <div class="wholesale-stepper" role="group" aria-label="Quantity for ${v.colour} ${v.size}">
            <button type="button" data-v-minus aria-label="Decrease quantity">−</button>
            <strong>${v.quantity}</strong>
            <button type="button" data-v-plus aria-label="Increase quantity">+</button>
          </div>
        </div>
      </article>`).join('');
    ...
    root.querySelectorAll('.wholesale-mix-card').forEach(card=>{
      ...
      card.querySelector('[data-v-size]').onchange=e=>{variants[i].size=e.target.value;renderVariants()};
      card.querySelector('[data-v-minus]').onclick=()=>{variants[i].quantity=Math.max(1,variants[i].quantity-1);renderVariants()};
      card.querySelector('[data-v-plus]').onclick=()=>{variants[i].quantity++;renderVariants()};
      card.querySelector('[data-v-remove]')?.addEventListener('click',()=>{variants.splice(i,1);renderVariants()});
    });
  }
  ...
  function render(){
    ...
    const u=new URL(location.href);u.searchParams.set('id',product.id);u.searchParams.set('mode',mode);u.searchParams.set('colour',colourSlug(colour));history.replaceState({},'',u);
    gallery();renderVariants();
  }

  function miniCart(item){
    let d=document.querySelector('[data-mini-cart]');
    if(!d){d=document.createElement('aside');d.className='mini-cart-drawer';d.dataset.miniCart='';document.body.appendChild(d)}
    d.innerHTML=`<button class="mini-cart-close" type="button" aria-label="Close">×</button><p class="eyebrow">Added to bag</p><div class="mini-cart-item"><img src="${item.image}" alt=""><div><strong>${item.name}</strong><span>${item.totalQuantity} piece${item.totalQuantity===1?'':'s'} · ${item.orderType}</span><small>${item.variants.map(v=>`${v.quantity} × ${v.colour} / ${v.size}`).join(' · ')}</small></div></div><div class="mini-cart-total"><span>Bag total</span><strong>${WGH.money(WGH.getCart().reduce((s,i)=>s+i.unitPrice*i.totalQuantity,0))}</strong></div><a class="button button-dark full" href="cart.html">View full cart</a><button class="text-button" type="button" data-keep-shopping>Keep shopping</button>`;
  ...
  $('[data-add-to-bag]').onclick=()=>{
    ...
    const imgs=(product.colourImages?.[itemColour]?.length?product.colourImages[itemColour]:product.images)||[];
    const item={key:`${product.id}-${Date.now()}`,productId:product.id,name:product.name,image:imgs[0],orderType:mode,unitPrice:mode==='wholesale'?product.wholesalePrice:product.retailPrice,totalQuantity:total,variants:mode==='wholesale'?variants.map(v=>({...v})):[{colour,size,quantity:qty}]};
    const cart=WGH.getCart();cart.push(item);WGH.saveCart(cart);miniCart(item);
  };

  if(product.available===false){
    const add=$('[data-add-to-bag]');add.disabled=true;add.textContent='Temporarily paused';
    const notify=document.createElement('form');notify.className='notify-me-form';notify.innerHTML='<p class="mini-label">Want this piece?</p><strong>Get an email when ordering reopens.</strong><div><input type="email" name="email" placeholder="Email address" required><button class="button button-outline" type="submit">Notify me</button></div>';add.insertAdjacentElement('afterend',notify);
    setTimeout(()=>{if(WGH.auth?.currentUser?.email&&!notify.elements.email.value)notify.elements.email.value=WGH.auth.currentUser.email},800);
    notify.onsubmit=async e=>{e.preventDefault();const btn=notify.querySelector('button'),email=notify.elements.email.value;await WGH.withLoading(btn,async()=>{await WGH.api('/notify',{email,productId:product.id});WGH.showToast('We will let you know when this piece is available.','success');notify.reset()},'Saving')};
  }
