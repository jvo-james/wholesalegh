(() => {
  ...
  const statuses=WGH.statuses,labels=WGH.statusLabels;let batches=[],orders=[],customers=[],accounts=[],categories=[],notifications=[],selectedBatch=null,adminAuth=null; const ADMIN_VIEWS=['overview','orders','transactions','international','analytics','batches','products','discounts','categories','wholesale','customers','accounts','abandoned','subscribers','alerts','activity','settings'];
  ...
  function openOrderDetail(number){
    ...
    const items=(o.items||[]).map((item,itemIndex)=>{
      const variants=(item.variants||[]).map((v,i)=>`<div class="order-variant-detail"><span class="variant-number">${i+1}</span><div><small>Quantity</small><strong>${Number(v.quantity||1)}</strong></div><div><small>Colour</small><strong>${escape(v.colour||'Not specified')}</strong></div><div><small>Size</small><strong>${escape(v.size||'Not specified')}</strong></div></div>`).join('');
      return `<article class="admin-order-product"><header><img src="${escape(item.image||'')}" alt=""><div><span>Item ${itemIndex+1}</span><h3>${escape(item.name||'Product')}</h3><p>${escape(item.mode||item.orderType||'retail')} order · ${Number(item.totalQuantity||1)} total piece${Number(item.totalQuantity||1)===1?'':'s'}</p></div></header><div class="order-variant-list">${variants||'<p class="muted-copy">No colour or size breakdown was saved for this item.</p>'}</div></article>`;
    }).join('');
  ...
  async function loadAlerts(){
    const [stored]=await Promise.all([adminApi('/notifications'),ensureOrders(),batches.length?Promise.resolve(batches):adminApi('/batches').then(x=>batches=x)]);notifications=stored||[];
    const operational=[];
  ...
  async function loadTransactions(){
    ...
    try{
      ...
      if(root)root.innerHTML=tx.map(o=>`<tr class="admin-click-row" data-order-detail="${escape(o.orderNumber)}"><td><strong>${escape(o.orderNumber)}</strong><small>${date(o.createdAt)}</small></td><td><strong>${escape(o.customerName||'Customer')}</strong><small>${escape(o.customerEmail||'')}</small></td><td><code class="transaction-ref">${escape(o.paymentReference||'Not recorded')}</code></td><td><span class="status-pill ${String(o.paymentStatus).toLowerCase()==='paid'?'':'warning'}">${escape(o.paymentStatus||'Recorded')}</span></td><td><strong>${WGH.money(o.total)}</strong></td><td>${date(o.createdAt)}</td></tr>`).join('')||'<tr><td colspan="6"><div class="admin-empty-row"><strong>No transactions found.</strong><span>If an order exists in Orders, press Refresh. Older orders are also included now.</span></div></td></tr>';
...
document.querySelector('[data-refresh-settings]')?.addEventListener('click',e=>WGH.withLoading(e.currentTarget,loadSettings,'Refreshing'));
  ...
  let notificationOpen=false;async function loadNotificationPreview(){try{
    ...
    const persistent=notifications.map(n=>({...n,icon:n.type==='order'?'fa-bag-shopping':'fa-bell'})),items=[...persistent,...live],unread=persistent.filter(x=>!x.read).length+live.length;
