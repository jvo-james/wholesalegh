...
let abandonTimer;const saveAbandoned=()=>{clearTimeout(abandonTimer);abandonTimer=setTimeout(()=>{const email=String(form.elements.email?.value||'').trim(),phone=String(form.elements.phone?.value||'').trim();if(!email&&!phone)return;WGH.api('/abandoned-cart',{email,phone,name:`${form.elements.firstName?.value||''} ${form.elements.lastName?.value||''}`.trim(),items:cart},{auth:!!WGH.auth?.currentUser}).catch(()=>{})},700)};form.addEventListener('input',saveAbandoned);
